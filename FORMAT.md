...
## Playing, checking, and the keys

Everything about *running* keynot -- the subcommands, `play` flags, the
full key table, the line highlight, the outline, the `!` shell and `e`
editor escapes, and how overflowing slides scroll -- lives in
[USAGE.md](USAGE.md). This document is only about what goes in the
file.
